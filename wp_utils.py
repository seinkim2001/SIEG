...
def load_splitted_data(args):
    ...
    if args.test_ratio==0.5:
        data_dir = os.path.join(cur_dir, 'wp_data/splitted_0_5/{}.mat'.format(data_name))
    else:
        data_dir = os.path.join(cur_dir, 'wp_data/splitted/{}.mat'.format(data_name))
...
def load_unsplitted_data(args):
    ...
    data_dir = os.path.join(cur_dir, 'wp_data/{}.mat'.format(args.dataset))
    print('Load data from: '+ data_dir)
    import scipy.io as sio
    net = sio.loadmat(data_dir)
...
def load_Planetoid_data(args):
    ...
    #dataset = Planetoid(root=cur_dir+'/wp_data/', name=args.data_name, transform=NormalizeFeatures())
    dataset = Planetoid(root=cur_dir+'/wp_data/', name=args.data_name)
    data = dataset[0]
    data.num_nodes = torch.max(data.edge_index)+1
    return data
# def load_Planetoid_data(args):
...
def set_init_attribute_representation(data,args):
    ...
    if data.x == None:
        ...
        if args.init_attribute =='spc':
            ...
            x = x.float()
        if args.init_attribute =='ones':  # this way
    ...
    if args.init_representation != None:
        ...
        if args.init_representation == 'gic':
            sys.path.append('%s/wp_software/GIC/' % args.cur_dir)
...
def prepare_data(args):
    ...
    if args.data_name in ('cora', 'citeseer', 'pubmed'):
        ...
        splitstr = 'wp_data/new_data_splits/'+datastr+'_split_0.6_0.2_'+split_index+'.npz'
    ...
    val_loader = DataLoader(val_graphs,batch_size=args.batch_size,shuffle=True,num_workers=args.num_workers)
    test_loader = DataLoader(test_graphs,batch_size=args.batch_size,shuffle=False,num_workers=args.num_workers)
    import pdb; pdb.set_trace()

    return train_graphs, val_graphs, test_graphs, train_loader, val_loader, test_loader, feature_results
...
def new_load_data(dataset_name, splits_file_path=None):
    ...
    graph_adjacency_list_file_path = os.path.join(cur_dir,'wp_data','new_data', dataset_name, 'out1_graph_edges.txt')
    graph_node_features_and_labels_file_path = os.path.join(cur_dir,'wp_data','new_data', dataset_name,
    ...
    splits_file_path = os.path.join(cur_dir,splits_file_path)  
